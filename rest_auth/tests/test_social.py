...
class TestSocialAuth(TestsMixin, TestCase):
    ...
    def test_email_clash_with_existing_account(self):
        ...
        # verify email
        email_confirmation = new_user.emailaddress_set.get(email=self.EMAIL).emailconfirmation_set.order_by('-created')[
            0
        ]
        self.post(
...
class TestSocialConnectAuth(TestsMixin, TestCase):
    ...
    def test_social_connect(self):
        ...
        self.incorrect_disconnect_url = reverse('social_account_disconnect', args=[999999999])
        self.post(self.incorrect_disconnect_url, status_code=404)

        self.disconnect_url = reverse('social_account_disconnect', args=[facebook_social_account_id])
